locals {
  compute_snapshot_if_storage_premium_query = <<-EOQ
		select
...
trigger "query" "detect_and_correct_compute_snapshot_if_storage_premium" {
  ...
  documentation = file("./compute/docs/detect_and_correct_compute_snapshot_if_storage_premium_trigger.md")
  tags          = merge(local.compute_common_tags, { class = "unused" })

  enabled  = var.compute_snapshot_if_storage_premium_trigger_enabled
  schedule = var.compute_snapshot_if_storage_premium_trigger_schedule
  database = var.database
  sql      = local.compute_snapshot_if_storage_premium_query

  capture "insert" {
    pipeline = pipeline.correct_compute_snapshot_if_storage_premium
    args = {
...
pipeline "detect_and_correct_compute_snapshot_if_storage_premium" {
  title         = "Detect & correct Compute snapshots with premium storage"
  description   = "Detects Compute snapshots with premium storage and runs your chosen action."
  documentation = file("./compute/docs/detect_and_correct_compute_snapshot_if_storage_premium.md")
  ...
  param "default_action" {
    ...
    default     = var.compute_snapshot_if_storage_premium_default_action
  }

  param "enabled_actions" {
    type        = list(string)
    description = local.description_enabled_actions
    default     = var.compute_snapshot_if_storage_premium_enabled_actions
  }
...
pipeline "correct_compute_snapshot_if_storage_premium" {
  ...
  documentation = file("./compute/docs/correct_compute_snapshot_if_storage_premium.md")
  tags          = merge(local.compute_common_tags, { class = "unused" })
...
variable "compute_snapshot_if_storage_premium_trigger_enabled" {
  type        = bool
  default     = false
  description = "If true, the trigger is enabled."
}

variable "compute_snapshot_if_storage_premium_trigger_schedule" {
  type        = string
  default     = "15m"
  description = "The schedule on which to run the trigger if enabled."
}

variable "compute_snapshot_if_storage_premium_default_action" {
  type        = string
  description = "The default action to use for the detected item, used if no input is provided."
  default     = "notify"
}

variable "compute_snapshot_if_storage_premium_enabled_actions" {
  type        = list(string)
