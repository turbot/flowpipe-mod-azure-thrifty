locals {
  kubernetes_cluster_exceeding_max_age_query = <<-EOQ
   	select
		...
		where
			date_part('day', now()-created_time) > ${var.kubernetes_cluster_exceeding_max_age_days};
  EOQ
}
...
trigger "query" "detect_and_correct_kubernetes_cluster_exceeding_max_age" {
  ...
  schedule = var.kubernetes_cluster_exceeding_max_age_trigger_schedule
  database = var.database
  sql      = local.kubernetes_cluster_exceeding_max_age_query

  capture "insert" {
    pipeline = pipeline.correct_kubernetes_cluster_exceeding_max_age
    args = {
...
pipeline "detect_and_correct_kubernetes_cluster_exceeding_max_age" {
  ...
  param "default_action" {
    ...
    default     = var.kubernetes_cluster_exceeding_max_age_default_action
  }

  param "enabled_actions" {
    type        = list(string)
    description = local.description_enabled_actions
    default     = var.kubernetes_cluster_exceeding_max_age_enabled_actions
  }
...
variable "kubernetes_cluster_exceeding_max_age_trigger_enabled" {
  type        = bool
  default     = false
  description = "If true, the trigger is enabled."
}

variable "kubernetes_cluster_exceeding_max_age_trigger_schedule" {
  type        = string
  default     = "15m"
  description = "The schedule on which to run the trigger if enabled."
}

variable "kubernetes_cluster_exceeding_max_age_default_action" {
  type        = string
  description = "The default action to use for the detected item, used if no input is provided."
  default     = "notify"
}

variable "kubernetes_cluster_exceeding_max_age_enabled_actions" {
  type        = list(string)
  description = "The list of enabled actions to provide to approvers for selection."
  default     = ["skip", "delete_cluster"]
}

variable "kubernetes_cluster_exceeding_max_age_days" {
  type        = number
