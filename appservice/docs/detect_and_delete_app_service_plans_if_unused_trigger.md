...
Azure AppService plan with no apps attached still cost money and should be deleted. This pipeline identifies AppService plan with no apps attached and either sends notifications or attempts predefined corrective actions.
...
```sh
flowpipe pipeline run detect_and_delete_app_service_plans_if_unused
```
...
```sh
flowpipe pipeline run detect_and_delete_app_service_plans_if_unused --host local --arg='approvers=["default"]'
```
...
```sh
flowpipe pipeline run detect_and_delete_app_service_plans_if_unused --arg='default_action="delete"'
```

However; if you have configured a non-empty list for your `approvers` variable, you will need to override it as below:

```sh
flowpipe pipeline run detect_and_delete_app_service_plans_if_unused --arg='approvers=[]' --arg='default_action="delete"'
```

This will attempt to apply the action to every detected item, if you're happy with this approach you could have this occur mmore frequently by either scheduling the command by yourself or enabling the associated [Query Trigger](https://hub.flowpipe.io/mods/turbot/azure_thrifty/triggers/azure_thrifty.trigger.query.detect_and_delete_app_service_plans_if_unused).