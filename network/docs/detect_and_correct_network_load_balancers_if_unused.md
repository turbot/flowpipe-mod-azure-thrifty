...
```sh
flowpipe pipeline run detect_and_correct_network_load_balancers_if_unused --host local --arg='approvers=["default"]'
```
...
```sh
flowpipe pipeline run detect_and_correct_network_load_balancers_if_unused --arg='default_action="delete_lb"'
```

However; if you have configured a non-empty list for your `approvers` variable, you will need to override it as below:

```sh
flowpipe pipeline run detect_and_correct_network_load_balancers_if_unused --arg='approvers=[]' --arg='default_action="delete_lb"'
```
